...
fun <T> checkType(args: T): String {
    val a = if(args is Int) "Yes! it's Integer" else
        if(args is String) "Yes! it's String" else
            if(args is Boolean)"Yes! it's Boolean" else
                if(args is Double)"Yes! it's Double" else
                    if(args is List<*>)"Yes! it's List" else
                        if(args is Map<*,*>)"Yes! it's Map"
                        else "null"
    return a
}
fun main() {
